...
void ACombatCharacter::Tick(float DeltaSeconds)
{
	...
	// Can also be done with (CurrentRotation - LastRotation).GetNormalized();
	DeltaRotation = UKismetMathLibrary::NormalizedDeltaRotator(CurrentRotator, LastRotation);
	LastRotation = CurrentRotator;
}

float ACombatCharacter::GetHorizontalSpeed() const
{
